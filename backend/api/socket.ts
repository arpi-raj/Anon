...
const wsClients = new Map<number, WebSocket>(); // Store client WebSockets
const connections = new Map<number, number>(); // Store active chat connections
...
export class WebSocketServerSingleton {
  ...
  static getInstance(server?: http.Server): WebSocketServerSingleton {
    if (!WebSocketServerSingleton.instance) {
      if (!server) {
        throw new Error(
          "Server instance required for WebSocket initialization."
        );
      }
  ...
  public registerClient(clientId: number, ws: WebSocket): boolean {
  ...
  public connectClients(from: number, to: number): boolean {
  ...
  public sendMessage(from: number, to: number, text: string): boolean {
    if (!wsClients.has(to)) {
  ...
  public removeClient(clientId: number): void {
    if (wsClients.has(clientId)) {
      ...
      if (connectedTo) {
        ...
        wsClients.get(connectedTo)?.send(
          JSON.stringify({
            type: "disconnected",
            partner: clientId,
          })
